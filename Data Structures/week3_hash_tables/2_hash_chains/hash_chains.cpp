...
public:
    explicit QueryProcessor(int bucket_coun) {
        bucket_count = bucket_coun;
    ...
    void processQuery(const Query& query) {
        if (query.type == "check") {
            ...
            vector<string>::iterator it = std::find(elems[hash_func(query.s)].begin(), elems[hash_func(query.s)].end(), query.s);
            if (query.type == "find")
